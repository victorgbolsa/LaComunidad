...
def snaptrade_connect():
    ...
    if reg_r.status_code == 200:
        ...
        if not user_secret:
            # No hay userSecret — borrar usuario de SnapTrade y re-registrar
            log.info("userSecret no encontrado — borrando usuario de SnapTrade y re-registrando...")
            del_extra = f"userId={user_id}"
            # Necesitamos un userSecret temporal para borrar — intentar con uno genérico
            # SnapTrade permite borrar sin userSecret usando solo userId
            del_r = snaptrade_request("DELETE", "/api/v1/snapTrade/deleteUser", None, del_extra)
            log.info(f"SnapTrade deleteUser: {del_r.status_code} {del_r.text[:100]}")

            # Intentar registrar de nuevo
            reg2_r = snaptrade_request("POST", "/api/v1/snapTrade/registerUser", {"userId": user_id})
            log.info(f"SnapTrade re-registerUser: {reg2_r.status_code} {reg2_r.text[:200]}")

            if reg2_r.status_code == 200:
                user_secret = reg2_r.json().get("userSecret", "")
                if user_secret and SUPABASE_KEY:
                    try:
                        h = {"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}",
                             "Content-Type": "application/json", "Prefer": "return=minimal"}
                ...
                return jsonify({"error": "No se pudo re-registrar el usuario en SnapTrade. Contacta con soporte."}), 500
    else:
...
def snaptrade_data():
    ...
    try:
        r = snaptrade_get("/api/v1/holdings")
        log.info(f"SnapTrade holdings: {r.status_code} {r.text[:100]}")
        if r.status_code == 200:
    ...
    try:
        r = snaptrade_get("/api/v1/activities", "days=365")
    ...
    try:
        ...
        r = snaptrade_get("/api/v1/performance/custom",
                          f"startDate=2020-01-01&endDate={end}&frequency=MONTHLY")
