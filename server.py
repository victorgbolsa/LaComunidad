...
def snaptrade_connect():
    ...
    # 1. Registrar usuario — clientId va también como query param
    reg_r = requests.post(
        "https://api.snaptrade.com/api/v1/snapTrade/registerUser",
        headers=snaptrade_sig(),
        params={"clientId": SNAPTRADE_CLIENT_ID},
        json={"userId": user_id},
        timeout=15
    )
    ...
    if reg_r.status_code == 200:
        ...
        # Guardar userSecret en Supabase para futuros logins
    ...
    elif reg_r.status_code == 409:
        # Usuario ya registrado — recuperar userSecret guardado
        user_secret = ""
        if SUPABASE_KEY:
            try:
                h = {"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}"}
                r = requests.get(f"{SUPABASE_URL}/rest/v1/snaptrade_users?user_id=eq.{user_id}&select=user_secret",
        ...
        if not user_secret:
            return jsonify({"error": "Usuario ya registrado pero userSecret no encontrado"}), 500
    else:
        return jsonify({"error": f"Error registrando usuario: {reg_r.text[:100]}"}), 500

    # 2. Generar link — userId y userSecret van como QUERY PARAMS junto con clientId
    link_r = requests.post(
        "https://api.snaptrade.com/api/v1/snapTrade/login",
        headers=snaptrade_sig(),
        params={"clientId": SNAPTRADE_CLIENT_ID, "userId": user_id, "userSecret": user_secret},
        timeout=15
    )
...
def snaptrade_data():
    ...
    try:
        r = requests.get("https://api.snaptrade.com/api/v1/holdings",
                         headers=snaptrade_sig(), params=params, timeout=15)
    ...
    try:
        r = requests.get("https://api.snaptrade.com/api/v1/activities",
                         headers=snaptrade_sig(), params={**params, "days": 365}, timeout=15)
