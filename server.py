...
@app.route("/snaptrade/price-history", methods=["OPTIONS"])
...
def verify_any_authenticated_user(req):
    ...
    user_id concreto — solo que exista una sesión válida. Devuelve el
    user_id real si es válido, o None si no lo es."""
...
def get_email_summary() -> dict:
    # FIX (11/07/2026): market_tracker.py corre en un servicio de Render
    # DISTINTO ("cron-dashboard") a este ("LaComunidad") — cada uno con su
    # propio disco. El fichero local email_summary.json que se leía aquí
    # NUNCA fue el mismo que escribía market_tracker.py, así que este
    # "return {}" era casi siempre lo que pasaba, y de ahí que el email
    # cayera siempre al fallback hardcodeado (NVDA/META/AXON/ORCL/CRWD/PLTR).
    # Ahora se lee de Supabase (app_state), que sí es compartido entre
    # servicios.
    if not SUPABASE_KEY:
        return {}
    try:
        h = {"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}"}
        r = requests.get(f"{SUPABASE_URL}/rest/v1/app_state?key=eq.email_summary&select=value",
        ...
        log.warning(f"Error leyendo email_summary de Supabase: {e}")
    return {}


def get_amplitud_resumen() -> dict:
    # NUEVO (04/09/2026): datos reales de amplitud (ADn, RASI, McClellan,
    # Línea AD, % sobre medias) que market_tracker.py guarda en Supabase
    # tras cada actualización. Antes, el email semanal no tenía NINGÚN
    # dato real de amplitud — el bloque "Amplitud de mercado" lo generaba
    # la IA solo a partir de precios de índices, sin números de amplitud
    # de verdad detrás.
    if not SUPABASE_KEY:
        return {}
    try:
        h = {"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}"}
        r = requests.get(f"{SUPABASE_URL}/rest/v1/app_state?key=eq.amplitud_resumen&select=value",
                          headers=h, timeout=10)
        if r.status_code == 200:
            rows = r.json()
            if rows:
        ...
        log.warning(f"Error leyendo amplitud_resumen de Supabase: {e}")
    return {}


@app.route("/api/amplitud-snapshot", methods=["GET"])
def api_amplitud_snapshot():
    """Expone el último dato COMPLETO de amplitudmercado.com (nyse/nasdaq/
    aaii/ii/diarios/bonos/fearGreed) que el cron del dashboard consiguió con
    éxito, guardado en Supabase (app_state, clave 'amplitud_completa') por
    guardar_resumen_amplitud_supabase() en market_tracker.py.

    Existe porque el bot de Telegram NO puede conectar directamente a
    amplitudmercado.com — se confirmó que la misma petición, con las mismas
    cabeceras, funciona bien desde Render (aquí) pero no desde fuera
    (bloqueo por IP/origen). El bot lee de aquí en su lugar.

    Sin protección (GET, solo lectura): no expone nada sensible, solo
    indicadores de mercado públicos (RASI, ADn, McClellan, Fear&Greed...).
    """
    if not SUPABASE_KEY:
        return jsonify({"error": "Supabase no configurado."}), 500
    try:
        h = {"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}"}
        r = requests.get(f"{SUPABASE_URL}/rest/v1/app_state?key=eq.amplitud_completa&select=value",
                          headers=h, timeout=10)
        if r.status_code == 200:
            rows = r.json()
            if rows:
                return jsonify({"datos": rows[0].get("value", {})})
        return jsonify({"error": "Sin snapshot disponible todavía."}), 404
    except Exception as e:
        log.warning(f"Error leyendo amplitud_completa de Supabase: {e}")
...
def generar_comentario_ia(datos_mercado: list, amplitud: dict = None) -> dict:
    ...
    # NUEVO (04/09/2026): antes el bloque "timing" (titulado "Amplitud de
    # mercado" en el email) se generaba SIN ningún dato real de amplitud —
    # la IA solo veía precios de índices y tenía que inventarse el
    # comentario de amplitud a partir de ahí. Ahora, si hay datos reales
    # disponibles (ADn, RASI, McClellan, Línea AD, % sobre medias, todos
    # del NYSE vía amplitudmercado.com), se los pasamos explícitamente y
    # le pedimos que base el comentario de "timing" en ELLOS, no en los
    # precios.
    amplitud = amplitud or {}
    lineas_amplitud = []
    if amplitud.get("adn") is not None:
        lineas_amplitud.append(f"- Línea ADn (NYSE, 0-100): {amplitud['adn']}")
    if amplitud.get("rasi") is not None:
        lineas_amplitud.append(f"- RASI (suma acumulada McClellan): {amplitud['rasi']}")
    if amplitud.get("mcclellan") is not None:
        lineas_amplitud.append(f"- Oscilador McClellan: {amplitud['mcclellan']}")
    if amplitud.get("ad_line") is not None:
        tendencia_ad = ""
        if amplitud.get("ad_line_anterior") is not None:
            tendencia_ad = " (subiendo)" if amplitud["ad_line"] > amplitud["ad_line_anterior"] else " (bajando)"
        lineas_amplitud.append(f"- Línea AD acumulada (NYSE): {amplitud['ad_line']}{tendencia_ad}")
    if amplitud.get("pct_abv50") is not None:
        lineas_amplitud.append(f"- % de valores del NYSE sobre su media de 50 sesiones: {amplitud['pct_abv50']}%")
    if amplitud.get("pct_abv200") is not None:
        lineas_amplitud.append(f"- % de valores del NYSE sobre su media de 200 sesiones: {amplitud['pct_abv200']}%")
    if amplitud.get("coppock") is not None:
        signo = "positivo" if amplitud["coppock"] >= 0 else "negativo"
        sube = "subiendo" if amplitud.get("coppock_subiendo") else "bajando/plano"
        lineas_amplitud.append(f"- Coppock Curve mensual (S&P 500): {amplitud['coppock']} ({signo}, {sube})")

    bloque_amplitud = ""
    if lineas_amplitud:
        fuente = amplitud.get("fuente", "")
        bloque_amplitud = (
            "\n\nDatos REALES de amplitud de mercado" + (f" ({fuente})" if fuente else "") + ":\n"
            + "\n".join(lineas_amplitud)
            + "\n\nEl comentario de \"timing\" (amplitud de mercado) DEBE basarse en estos números "
            "de amplitud, no en los precios de los índices — menciona explícitamente el que consideres "
            "más relevante de la semana (ADn, McClellan, RASI, Línea AD o % sobre medias)."
        )
...
def enviar_email_semanal(test_email: str = "") -> dict:
    ...
    amplitud_resumen = get_amplitud_resumen()
    ...
    comentarios = generar_comentario_ia(datos_mercado, amplitud_resumen)
...
def snaptrade_data():
    ...
    try:
        ...
        log.info(f"SnapTrade positions: {r.status_code} {r.text[:1200]}")
        if r.status_code == 200:
    ...
    try:
        ...
        log.info(f"SnapTrade balances: {r.status_code} {r.text[:1000]}")
        if r.status_code == 200:
            result["balances"] = r.json()
    except Exception as e:
        log.warning(f"SnapTrade balances error: {e}")

    # 4. Órdenes de la cuenta — CONFIRMADO (10/07/2026, log real): probamos
    # 5 llamadas con startDate/endDate distintos cubriendo el año, y las 5
    # devolvieron EXACTAMENTE las mismas 37 órdenes — SnapTrade ignora esos
    # parámetros en este endpoint, siempre devuelve el mismo paquete
    # completo que tiene sincronizado. Así que 37 es (con toda probabilidad)
    # el historial completo disponible para esta cuenta, no un límite
    # nuestro — probablemente relacionado con la fecha en que se conectó el
    # broker (SnapTrade no siempre trae histórico anterior a la conexión).
    # Volvemos a una sola llamada: las ventanas no aportaban nada real,
    # solo gastaban 5x peticiones para obtener lo mismo.
    try:
        r = snaptrade_get(f"/api/v1/accounts/{account_id}/orders", "state=all&days=90")
        if r.status_code == 200:
            batch = r.json()
            result["orders"] = batch if isinstance(batch, list) else []
            # Volcado de una orden completa (no solo el conteo) — necesario
            # para comprobar si existe (y con qué nombre) el campo de
            # comisión en "orders", que es distinto de "activities".
            muestra = result["orders"][0] if result["orders"] else {}
            log.info(f"SnapTrade orders: {r.status_code} count={len(result['orders'])} muestra={muestra}")
        else:
            log.warning(f"SnapTrade orders: {r.status_code} {r.text[:200]}")
            result["orders"] = []
    except Exception as e:
        log.warning(f"SnapTrade orders error: {e}")

    # 5. Actividades de la cuenta (depósitos/retiradas/dividendos/trades) —
    # NUEVO (09/07/2026), AMPLIADO (13/07/2026), PAGINADO (13/08/2026):
    # CONFIRMADO con datos reales que SnapTrade pagina la respuesta —
    # {data:[...], pagination:{limit,offset,total}} — y "total" puede ser
    # mucho más alto que los 1000 que trae de golpe (ej. 1777 reales, solo
    # 1000 en la primera página). Sin esto, nos dejábamos hasta un 44% del
    # historial real por el camino sin saberlo.
    try:
        end_date = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        start_date = f"{datetime.now(timezone.utc).year - 2}-01-01"
        todas_las_activities = []
        offset = 0
        limit = 1000
        total_real = None
        n_paginas = 0
        while True:
            r = snaptrade_get(
                ...
                f"startDate={start_date}&endDate={end_date}&offset={offset}&limit={limit}"
            )
            n_paginas += 1
            if r.status_code != 200:
                log.warning(f"SnapTrade activities (página {n_paginas}, offset={offset}): {r.status_code} {r.text[:300]}")
                break
            body = r.json()
            pagina = body.get("data", body if isinstance(body, list) else [])
            todas_las_activities.extend(pagina)
            paginacion = body.get("pagination", {}) if isinstance(body, dict) else {}
            total_real = paginacion.get("total", len(pagina))
            if not pagina or len(todas_las_activities) >= total_real or n_paginas >= 10:  # tope de seguridad
                break
            offset += limit
        log.info(f"SnapTrade activities: {n_paginas} páginas, {len(todas_las_activities)}/{total_real} actividades traídas")
        result["activities"] = todas_las_activities
    except Exception as e:
...
# ══════════════════════════════════════════════════════════════════════════════
#  PRECIOS HISTÓRICOS PARA LA CURVA DE EQUITY (13/08/2026) — ENDPOINT
#  SEPARADO A PROPÓSITO. Antes esto vivía DENTRO de /snaptrade/data y
#  bloqueaba TODA la respuesta (incluida Fiscalidad) si Polygon tardaba en
#  algún ticker — así se rompió Fiscalidad la última vez. Ahora es su
#  propio endpoint: si esto va lento o falla, Mi Broker/Fiscalidad/Renta
#  siguen funcionando con normalidad, solo la curva de capital se ve
#  afectada. Las peticiones a Polygon van en PARALELO (antes en serie),
#  mucho más rápido.
# ══════════════════════════════════════════════════════════════════════════════
@app.route("/snaptrade/price-history", methods=["POST"])
def snaptrade_price_history():
    body = request.get_json(silent=True) or {}
    user_id = body.get("user_id", "")
    if not verify_supabase_user(request, user_id):
        return jsonify({"error": "No autorizado."}), 401
    tickers = body.get("tickers", [])
    fecha_min = body.get("start_date", "")
    if not fecha_min:
        return jsonify({"priceHistory": {}, "eurUsdHistory": {}})
    tickers = [t for t in tickers if isinstance(t, str) and len(t) <= 6][:30]  # tope de seguridad
    hoy = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    price_history = {}
    if tickers:
        try:
            with ThreadPoolExecutor(max_workers=8) as ex:
                resultados = ex.map(lambda tk: (tk, pg_daily_closes(tk, fecha_min, hoy)), tickers)
                for tk, closes in resultados:
                    if closes:
                        price_history[tk] = closes
        except Exception as e:
            log.warning(f"Error obteniendo precios históricos: {e}")

    # NUEVO (09/08/2026): histórico diario EUR/USD (mismo Polygon, ticker
    # forex "C:EURUSD") — hasta ahora TODO el sistema (Renta/IRPF y la
    # curva de equity de Mi Broker) sumaba/dibujaba directamente en dólares
    # sin convertir nunca a euros, o como mucho usaba el cambio de HOY
    # (válido para "cuánto vale mi cartera ahora", inválido para sumar
    # operaciones repartidas a lo largo del año — el cambio se movió mucho
    # en 2025). Se pide siempre que haya fecha_min, aunque no haya tickers
    # de acciones (así Renta puede pedir solo esto, sin listar tickers).
    eur_usd_history = {}
    try:
        eur_usd_history = pg_daily_closes("C:EURUSD", fecha_min, hoy)
    except Exception as e:
        log.warning(f"Error obteniendo histórico EUR/USD: {e}")

    log.info(f"Precios históricos: {len(price_history)}/{len(tickers)} tickers, {len(eur_usd_history)} días EUR/USD, desde {fecha_min}")
    return jsonify({"priceHistory": price_history, "eurUsdHistory": eur_usd_history})
