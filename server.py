...
def snaptrade_data():
    ...
    # 4. Órdenes recientes de la cuenta — NUEVO (10/07/2026): SnapTrade
    # confirmó por soporte que "days" se limita a 90 como máximo, aunque
    # pidiéramos 365 — por eso solo veíamos ~33 operaciones (últimos 3
    # meses), no el año completo. Solución: varias llamadas encadenadas de
    # 90 días cada una, cubriendo el año, usando startDate/endDate (mismo
    # patrón que ya funciona en "activities"). NO verificado al 100% que
    # startDate/endDate sea el nombre exacto de parámetro para "orders"
    # específicamente — revisa el log tras desplegar esto.
    try:
        all_orders, seen_ids = [], set()
        now_utc = datetime.now(timezone.utc)
        window_start = now_utc - timedelta(days=365)
        cursor = window_start
        n_calls = 0
        while cursor < now_utc and n_calls < 6:  # tope de seguridad, ~365/90 ≈ 5 ventanas
            window_end = min(cursor + timedelta(days=90), now_utc)
            qs = f"state=all&startDate={cursor.strftime('%Y-%m-%d')}&endDate={window_end.strftime('%Y-%m-%d')}"
            r = snaptrade_get(f"/api/v1/accounts/{account_id}/orders", qs)
            n_calls += 1
            if r.status_code == 200:
                batch = r.json()
                if isinstance(batch, list):
                    nuevas_en_ventana = 0
                    for o in batch:
                        oid = o.get("brokerage_order_id") or o.get("id")
                        if not oid or oid not in seen_ids:
                            if oid: seen_ids.add(oid)
                            all_orders.append(o)
                            nuevas_en_ventana += 1
                    # NUEVO: log por ventana — esto es lo que hace falta ver
                    # para saber si el problema es paginación interna de
                    # SnapTrade (ventanas devuelven pocas de las muchas que
                    # debería haber) o profundidad de sincronización (las
                    # ventanas más antiguas vienen vacías del todo).
                    log.info(f"SnapTrade orders ventana {cursor.date()}–{window_end.date()}: "
                             f"{len(batch)} recibidas, {nuevas_en_ventana} nuevas (no repetidas)")
                else:
                    log.warning(f"SnapTrade orders ventana {cursor.date()}–{window_end.date()}: respuesta no es una lista: {batch}")
            else:
                log.warning(f"SnapTrade orders window {cursor.date()}–{window_end.date()}: {r.status_code} {r.text[:200]}")
            cursor = window_end
        log.info(f"SnapTrade orders (año completo, {n_calls} llamadas): count={len(all_orders)}")
