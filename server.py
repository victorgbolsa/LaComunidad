...
def snaptrade_connect():
    ...
    import hmac, hashlib, uuid, datetime as dt
    ...
    # Generar un user_secret determinista para este usuario
    user_secret = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"{SNAPTRADE_CLIENT_ID}:{user_id}"))

    def snaptrade_headers(extra_sig=""):
        """Genera headers de autenticación SnapTrade."""
        ts = str(int(dt.datetime.now(timezone.utc).timestamp()))
        msg = f"{SNAPTRADE_CLIENT_ID}{ts}{extra_sig}"
        sig = hmac.new(SNAPTRADE_CONSUMER_KEY.encode(), msg.encode(), hashlib.sha256).hexdigest()
        return {"clientId": SNAPTRADE_CLIENT_ID, "timestamp": ts, "signature": sig,
                "Content-Type": "application/json"}

    # 1. Registrar usuario (si ya existe SnapTrade devuelve 200 igualmente)
    reg_r = requests.post(
        "https://api.snaptrade.com/api/v1/snapTrade/registerUser",
        headers=snaptrade_headers(),
        json={"userId": user_id},
        timeout=15
    )
    log.info(f"SnapTrade registerUser: {reg_r.status_code}")

    # 2. Generar link de conexión — userId y userSecret van en el body
    link_r = requests.post(
        "https://api.snaptrade.com/api/v1/snapTrade/login",
        headers=snaptrade_headers(),
        json={"userId": user_id, "userSecret": user_secret},
        timeout=15
    )
    ...
    if link_r.status_code == 200:
        ...
        return jsonify({"error": f"SnapTrade error {link_r.status_code}: {link_r.text[:100]}"}), 500
...
def snaptrade_data():
    ...
    def snaptrade_headers():
        ts = str(int(dt.datetime.now(timezone.utc).timestamp()))
        msg = f"{SNAPTRADE_CLIENT_ID}{ts}"
        sig = hmac.new(SNAPTRADE_CONSUMER_KEY.encode(), msg.encode(), hashlib.sha256).hexdigest()
        return {"clientId": SNAPTRADE_CLIENT_ID, "timestamp": ts, "signature": sig,
                "Content-Type": "application/json"}
    ...
    # Posiciones (holdings)
    try:
        r = requests.get("https://api.snaptrade.com/api/v1/holdings",
                         headers=snaptrade_headers(), params=params, timeout=15)
        log.info(f"SnapTrade holdings: {r.status_code}")
        if r.status_code == 200:
            result["holdings"] = r.json()
    except Exception as e:
        log.warning(f"SnapTrade holdings error: {e}")

    # Historial
    try:
        r = requests.get("https://api.snaptrade.com/api/v1/activities",
                         headers=snaptrade_headers(), params={**params, "days": 365}, timeout=15)
        log.info(f"SnapTrade activities: {r.status_code}")
        if r.status_code == 200:
    ...
    try:
        r = requests.get(
            ...
            headers=snaptrade_headers(),
            params={**params, "startDate": "2020-01-01",
                    "endDate": dt.date.today().isoformat(), "frequency": "MONTHLY"},
            timeout=15
        )
        log.info(f"SnapTrade performance: {r.status_code}")
        if r.status_code == 200:
            result["performance"] = r.json()
    except Exception as e:
        log.warning(f"SnapTrade performance error: {e}")
