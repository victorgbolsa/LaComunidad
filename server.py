...
def snaptrade_connect():
    ...
    # Registrar usuario en SnapTrade si no existe
    timestamp = str(int(dt.datetime.now(timezone.utc).timestamp()))
    sig_content = f"{SNAPTRADE_CLIENT_ID}{timestamp}"
    signature = hmac.new(
    ...
    # Intentar registrar el usuario (si ya existe, SnapTrade lo ignora)
    user_secret = str(uuid.uuid5(uuid.NAMESPACE_DNS, user_id))
    reg_r = requests.post(
        "https://api.snaptrade.com/api/v1/snapTrade/registerUser",
        headers=headers,
        json={"userId": user_id, "rsaPublicKey": ""},
        timeout=15
    )
    ...
    if link_r.status_code == 200:
        data_resp = link_r.json()
        return jsonify({
            "redirectURI": data_resp.get("redirectURI", ""),
            "userSecret": user_secret
        })
    else:
        log.error(f"SnapTrade login error: {link_r.status_code} {link_r.text[:200]}")
        return jsonify({"error": "Error generando link de conexión"}), 500
...
def snaptrade_data():
    ...
    try:
        r = requests.get(
            ...
            headers=headers, params={**params, "days": 365}, timeout=15
        )
        if r.status_code == 200:
