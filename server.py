...
def snaptrade_price_history():
    ...
    if not tickers or not fecha_min:
        return jsonify({"priceHistory": {}})
    ...
    log.info(f"Precios históricos: {len(price_history)}/{len(tickers)} tickers, desde {fecha_min}")
    return jsonify({"priceHistory": price_history})
